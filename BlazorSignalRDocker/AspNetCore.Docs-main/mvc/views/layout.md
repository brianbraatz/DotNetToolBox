...
Views and pages can use Razor directives to import namespaces and use [dependency injection](BlazorSignalRDocker/AspNetCore.Docs-main/aspnetcore/mvc/views/dependency-injection.md). Directives shared by many views may be specified in a common `_ViewImports.cshtml` file. The `_ViewImports` file supports the following directives:
